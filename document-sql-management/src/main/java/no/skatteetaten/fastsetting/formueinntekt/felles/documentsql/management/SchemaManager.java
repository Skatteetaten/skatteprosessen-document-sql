            ...
            if (exists) {
                try (PreparedStatement ps = conn.prepareStatement(
                    ...
                    if (force) {
                        ...
                        context.toDispatcher(name, e.getLength(), factory, pathResolver, rootResolver).drop(
                            dataSource,
                            LOGGER::debug,
                            exception -> factory.checkError(false, exception.getErrorCode())
                        );
                        onForce.accept(name);
                ...
                try {
                    ...
                    try {
                        ...
                        if (force && factory.checkError(true, e.getErrorCode())) {
                            LOGGER.info("Found conflicting tables for {} - attempting to replace schema", name);
                            try {
                                dispatcher.drop(dataSource, LOGGER::debug, exception -> factory.checkError(false, exception.getErrorCode()));
                            } catch (SQLException suppressed) {
                                if (!factory.checkError(false, suppressed.getErrorCode())) {
